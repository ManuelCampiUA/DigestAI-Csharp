...
namespace DigestAI_Csharp
{
    public class ProjectDigestGenerator
    {
        ...
        private readonly HashSet<string> _includedExtensions = new(StringComparer.OrdinalIgnoreCase)
        {
        ...
        private readonly HashSet<string> _excludedFilePatterns = new(StringComparer.OrdinalIgnoreCase)
        {
        ...
        private readonly HashSet<string> _priorityIncludes = new(StringComparer.OrdinalIgnoreCase)
        {
        ...
        public async Task GenerateAsync(string projectPath, string outputFilePath)
        {
            ...
            // Header del Markdown
            markdownBuilder.AppendLine("# AI Project Digest");
            markdownBuilder.AppendLine($"Generated on: {DateTime.Now:yyyy-MM-dd HH:mm:ss}");
            markdownBuilder.AppendLine($"Project path: `{projectPath}`");
            ...
            try
            {
                await File.WriteAllTextAsync(outputFilePath, markdownBuilder.ToString());
            }
            catch (Exception ex)
            {
        ...
        private IEnumerable<string> GetRelevantFiles(string projectPath)
        {
            return Directory.EnumerateFiles(projectPath, "*.*", SearchOption.AllDirectories)
                .Where(filePath =>
                {
                    ...
                    string relativeDirectory = Path.GetDirectoryName(Path.GetRelativePath(projectPath, filePath));
        ...
        private bool IsFileExcludedByPattern(string fileName)
        {
            ...
            foreach (var pattern in _excludedFilePatterns)
            {
                if (pattern.Contains("*")) // Solo se il pattern contiene un wildcard
                {
                    ...
                    if (pattern.StartsWith("*.") && fileName.EndsWith(pattern.Substring(1), StringComparison.OrdinalIgnoreCase)) // Es. "*.log" -> ".log"
                    {
                        return true;
                    }
